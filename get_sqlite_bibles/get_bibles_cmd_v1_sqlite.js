...
async function populateAllVersesForTranslation(db, bibleScraperInstance, progressBar) {
    ...
    for (const bookIndex in bookList) {
        ...
        progressBar.update(bookNumber, { bookName: long_name });
    ...
    progressBar.update(bookList.length + 1, { bookName: "Done!" }); // Завершуємо прогрес для цієї версії
    await runQuery(db, "COMMIT");
}
...
async function main() {
    ...
    // Створюємо MultiBar для керування всіма індикаторами прогресу
    const multibar = new cliProgress.MultiBar({
        clearOnComplete: false,
        hideCursor: true,
        format: ' {bar} | {language} ({version}) | {value}/{total} | {bookName}'
    }, cliProgress.Presets.shades_classic);

    // Функція для обмеження кількості одночасних потоків
    async function runWithConcurrency(poolLimit, allTasks, mainProgressBar) {
        const results = [];
        const executing = [];
        let taskIndex = 0;

        for (const task of allTasks) {
            // Запускаємо завдання і після його завершення оновлюємо головний прогрес-бар
            const p = Promise.resolve()
                .then(() => task(taskIndex++))
                .finally(() => mainProgressBar.increment());
            results.push(p);

            if (poolLimit <= allTasks.length) {
    ...
    const totalProgressBar = new cliProgress.SingleBar({
        format: 'Progress |{bar}| {percentage}% || {value}/{total} versions || ETA: {eta_formatted}',
    ...
    const processVersion = async (langKey, version, workerId) => {
        const logPrefix = `[Worker ${workerId + 1}]`;
        ...
        const progressBar = multibar.create(BibleScraper.BOOKS.length + 1, 0, {
            language: langKey.split(' - ')[0],
        ...
        try {
            ...
            await populateAllVersesForTranslation(db, bibleScraper, progressBar);
        } catch (error) {
            ...
            setTimeout(() => progressBar.stop(), 500);
        }
    };
    ...
    languages.forEach(([langKey, versions]) => {
        versions.forEach(version => {
            tasks.push((workerId) => processVersion(langKey, version, workerId));
        });
    });

    // Запускаємо обробку з обмеженням у 15 одночасних потоків
    const CONCURRENCY_LIMIT = 30;
    console.log(`\nStarting processing with ${CONCURRENCY_LIMIT} workers...`);
